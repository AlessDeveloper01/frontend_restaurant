        ...
        <>
            <>
                <div className={`card border border-primary`}>
                    <button
                        className="w-full p-6 bg-primary text-white font-bold text-center uppercase"
                        onClick={backPage}>
                        Regresar a la lista de ordenes
                    </button>
                    <div className="p-6">
                        <div className="flex justify-between items-center">
                            <h3 className="lg:text-2xl font-bold text-secondary dark:text-white mb-2">
                                Orden #{orden.id} - Mesero: {orden.mesero}
                            </h3>
                            <p className="lg:text-2xl font-bold text-secondary dark:text-white mb-2">
                                {formatDateTime(orden.date)}
                            </p>
                        </div>
                        <p className="mt-3 text-gray-800 dark:text-gray-400 mb-3 uppercase font-bold text-xl">
                            Productos Ordenados
                        </p>
                        {orden.orderProducts.map((item) => (
                            <div
                                ...
                                <li className="mb-1 text-lg" key={item.product.id}>
                                    {item.product.name} -{" "}
                                    ...
                                    <span className="font-black text-primary">
                                        {formatMxCurrency(item.product.price)}
                                    </span>
                                </li>
                            </div>
                        ))}
                        <div className="mt-3 text-gray-800 dark:text-gray-400 mb-3 uppercase font-bold text-right text-xl">
                            Total: {formatMxCurrency(orden.total)}
                        </div>
                        <div className="flex gap-3.5 flex-col uppercase font-black lg:grid lg:grid-cols-2">
                            <button
                                className={`btn bg-warning text-white mt-2 flex-1 uppercase font-black p-5`}
                                onClick={handleOrdenEdit}>
                                Editar
                            </button>
                            <button
                                className={`btn bg-danger text-white mt-2 flex-1 uppercase font-black p-5`}
                                onClick={() => {
                                    setIdOrden(orden.id);
                            ...
                            <Link
                                className={`btn bg-primary text-white mt-2 flex-1 p-5`}
                                to={`/print/${orden.id}`}>
                                Imprimir
                            </Link>
                            <button
                                className={`btn bg-success text-white mt-2 flex-1 uppercase font-black p-5`}
                                onClick={handleOrdenLista}>
                                Marcar como lista
                            </button>
                        </div>
                    </div>
